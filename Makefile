...
$(INST)/openstack-full-contrail.done: openstack-full-contrail.install $(INST)/openstack-full.done
	./openstack-full-contrail.install $(INST)/openstack-full $(INST)/openstack-full-contrail $(VERS)
...
$(INST)/install-server.done: install-server.install $(INST)/openstack-common.done $(SDIR)/build/deploy.install $(SDIR)/ansible/edeploy-install.yml postgresql-server.install puppetdb-server.install jenkins.install logcollector.install monitor-server.install tempest.install
...
$(INST)/base.done: $(ARCHIVE)/$(BVERS)/base-$(BVERS).edeploy
	rm -rf $(INST)/base
	mkdir -p $(INST)/base
	tar zxf $(ARCHIVE)/$(BVERS)/base-$(BVERS).edeploy --xattrs --selinux -C $(INST)/base
